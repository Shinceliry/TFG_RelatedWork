...
def get_motion_coef(coef_dict, rot_repr, with_global_pose=False, norm_stats=None):
    if norm_stats is not None:
        ...
        coef_dict = {k: ((coef_dict[k] - norm_stats[f'{k}_mean']) / norm_stats[f'{k}_std']).float() for k in keys}

    pose_coef = get_pose_input(coef_dict, rot_repr, with_global_pose).float() 
    return torch.cat([coef_dict['exp'].float(), pose_coef.float()], dim=-1)


def get_coef_dict(motion_coef, shape_coef=None, denorm_stats=None, with_global_pose=False, rot_repr='aa'):
    coef_dict = {
        'exp': motion_coef[..., :50].float()
    }
    if rot_repr == 'aa':
        if with_global_pose:
            coef_dict['pose'] = motion_coef[..., 50:].float()
        else:
            placeholder = torch.zeros_like(motion_coef[..., :3])
            coef_dict['pose'] = torch.cat([placeholder, motion_coef[..., -1:]], dim=-1).float()
    ...
    if shape_coef is not None:
        ...
        coef_dict['shape'] = shape_coef.float()
...
def coef_dict_to_vertices(coef_dict, flame, rot_repr='aa', ignore_global_rot=False, flame_batch_size=512):
    ...
    for i in range(0, n_samples, flame_batch_size):
        ...
        if rot_repr == 'aa':
            vert, _, _ = flame(
                batch_coef_dict['shape'].float(), batch_coef_dict['exp'].float(), batch_coef_dict['pose'].float(),
...
def compute_loss(args, is_starting_sample, shape_coef, motion_coef_gt, noise, target, prev_motion_coef, coef_stats,
    ...
    if args.target == 'noise':
        ...
        if args.l_vert > 0 or args.l_vel > 0:
            ...
            if args.rot_repr == 'aa':
                verts_gt, _, _ = flame(coef_gt['shape'].view(-1, 100).float(), coef_gt['exp'].view(-1, 50).float(),
                                       coef_gt['pose'].view(-1, 6).float(), return_lm2d=False, return_lm3d=False)
                verts_pred, _, _ = flame(coef_pred['shape'].view(-1, 100).float(), coef_pred['exp'].view(-1, 50).float(),
                                         coef_pred['pose'].view(-1, 6).float(), return_lm2d=False, return_lm3d=False)
            else:
...
def truncate_motion_coef_and_audio(audio, motion_coef, n_motions, audio_unit=640, pad_mode='zero'):
    ...
    # truncate coef dict
    coef_dict_trunc = _truncate_coef_dict(coef_dict, end_idx, pad_mode=pad_mode)
    motion_coef_trunc = torch.cat([coef_dict_trunc['exp'].float(), coef_dict_trunc['pose_any'].float()], dim=-1)
...
def nt_xent_loss(feature_a, feature_b, temperature):
    ...
    features = torch.cat([feature_a.float(), feature_b.float()], dim=0)
