...
def train(args, model: DiffTalkingHead, style_enc: Optional[StyleEncoder], train_loader, val_loader, optimizer,
    ...
    for it in pbar:
        ...
        # wandbログの追加
        wandb.log({
            'train/loss': np.mean(loss_log['loss']),
            'train/noise': np.mean(loss_log['noise']),
            'train/vert': np.mean(loss_log['vert']) if args.target == 'sample' and args.l_vert > 0 else None,
            'train/vel': np.mean(loss_log['vel']) if args.target == 'sample' and args.l_vel > 0 else None,
            'train/smooth': np.mean(loss_log['smooth']) if args.target == 'sample' and args.l_smooth > 0 else None,
            'train/head_angle': np.mean(loss_log['head_angle']) if args.target == 'sample' and predict_head_pose and args.l_head_angle > 0 else None,
            'train/head_vel': np.mean(loss_log['head_vel']) if args.target == 'sample' and predict_head_pose and args.l_head_vel > 0 else None,
            'train/head_smooth': np.mean(loss_log['head_smooth']) if args.target == 'sample' and predict_head_pose and args.l_head_smooth > 0 else None,
            'train/head_trans': np.mean(loss_log['head_trans']) if args.target == 'sample' and predict_head_pose and args.l_head_trans > 0 else None,
        ...
        # wandbのログに追加
        if it % args.val_iter == 0 and it != 0:
            wandb.log({
                'val/loss': np.mean(loss_log['loss']),
                'val/noise': np.mean(loss_log['noise']),
                'val/vert': np.mean(loss_log['vert']) if args.target == 'sample' and args.l_vert > 0 else None,
                'val/vel': np.mean(loss_log['vel']) if args.target == 'sample' and args.l_vel > 0 else None,
                'val/smooth': np.mean(loss_log['smooth']) if args.target == 'sample' and args.l_smooth > 0 else None,
                'val/head_angle': np.mean(loss_log['head_angle']) if args.target == 'sample' and predict_head_pose and args.l_head_angle > 0 else None,
                'val/head_vel': np.mean(loss_log['head_vel']) if args.target == 'sample' and predict_head_pose and args.l_head_vel > 0 else None,
                'val/head_smooth': np.mean(loss_log['head_smooth']) if args.target == 'sample' and predict_head_pose and args.l_head_smooth > 0 else None,
                'val/head_trans': np.mean(loss_log['head_trans']) if args.target == 'sample' and predict_head_pose and args.l_head_trans > 0 else None,
            })
...
def main(args, option_text=None):
    ...
    # wandbの初期化
    wandb.init(project="DiffTalkingHead", config=args, name=args.exp_name)
    ...
    if args.mode == 'train':
        ...
        # Dataset
        train_dataset = LmdbDataset(data_root, data_root / 'train.txt', coef_stats_file, args.fps, args.sr, args.n_motions,
                                    rot_repr=args.rot_repr)
        val_dataset = LmdbDataset(data_root, data_root / 'val.txt', coef_stats_file, args.fps, args.sr, args.n_motions,
                                  rot_repr=args.rot_repr)
        train_loader = data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True,
                                       num_workers=args.num_workers, pin_memory=True)
        val_loader = data.DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False,
        ...
        # トレーニング中のlossをログに追加
        train(args, model, style_enc, train_loader, val_loader, optimizer, exp_dir / 'checkpoints', scheduler, writer, flame)
        
        # wandbにファイルを保存
        wandb.save(str(exp_dir / 'checkpoints') + '/*.pt')
