...
class LmdbDataset(data.Dataset):
    def __init__(self, lmdb_dir, split_file, coef_stats_file=None, coef_fps=25, sr=16000., n_motions=100, crop_strategy='random', rot_repr='aa'):
        ...
        self.audio_unit = sr / self.coef_fps  # num of samples per frame
    ...
    def __getitem__(self, index):
        ...
        if seq_len < self.coef_total_len:
            return self.__getitem__((index + 1) % len(self.entries))
        ...
        with self.lmdb_env.begin(write=False) as txn:
            for clip_idx in range(start_clip, end_clip):
                ...
                try:
                    entry_data = txn.get(key)
                    if entry_data is None:
                        # print(f"Warning: Entry {key.decode()} not found in LMDB. Skipping.")
                        continue

                    entry = pickle.loads(entry_data)
                except (pickle.UnpicklingError, EOFError, AttributeError) as e:
                    # print(f"Error loading entry {key.decode()}: {e}. Skipping this entry.")
                    continue
        ...
        if coef_dict['exp'].shape[0] != self.coef_total_len:
            # print(f"Skipping: Key={self.entries[index]}, Expected length={self.coef_total_len}, Actual length={coef_dict['exp'].shape[0]}")
            return self.__getitem__((index + 1) % len(self.entries))
        
        audio = torch.cat(audio, dim=0)
        # assert audio.shape[0] == round(self.coef_total_len * self.audio_unit), f'Invalid audio length: {audio.shape[0]}'
        expected_len = round(self.coef_total_len * self.audio_unit)
        if audio.shape[0] < expected_len:
            padding = expected_len - audio.shape[0]
